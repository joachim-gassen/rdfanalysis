...
As case we assess the impact of real Gross Domestic Product (GDP) per capita on life expectancy. This association has become known as the [Preston Curve](https://en.wikipedia.org/wiki/Preston_curve) ([Preston, Pop Studies, 1975](https://www.tandfonline.com/doi/abs/10.1080/00324728.1975.10410201)). We will estimate the respective coefficient for GDP per capita in a multiple regression framework on a panel data set of country-year data provided by the World Bank.

While the analysis it not meant to contribute to the question whether there is a causal link between these two constructs, I design the analysis such that identifying assumptions for such a causal interpretation of its findings are explicit.
...
The first function reads or generates data, the last function generates results. In principle, the `rdfanalysis` package is agnostic about the data structure that constitutes a result. In most cases, the result will take the form of one or several confidence intervals. In our case, it is the one $95 \%$ confidence interval for the GDP per capita coefficient, provided by three variables:
- the estimate (`est`)
...
Each step will be implemented by a function. To set up such a functional structure, you use the command `define_design()`. This function creates a series of function templates in a directory of your choice. Existing files are not over-written. You can choose whether you want your functions all in one R file or whether you want to generate separtate files. I am going with separate files here.
...
- `specify_model()`: `model_type` determines whether the model should be estimated as level-level, log-level, level-log or log-log specification.
...
$lifeexpectancy = \beta_0 + es\log(gdp\_capita) + 10\log(resdevelop_gdp) + 5log(unemployment) + \epsilon$

with $\beta_0$ being based on country-level base-lines that are adjusted for sample means of the independent variables.
The effect `es` of ` log(gdp_capita)` on `life_expectancy` is our main coefficient of interest. Like the `read_data()` function, `sim_data()` returns a path to a temporary CSV file containing the simulated data.
...
sim_data <- function(countries = 75, es = 1, years = 15) {
  ...
  df$lifeexpectancy <- rep(base_le, years) - 
    10 * mean(log(df$resdevelop_gdp)) + 
    5 * mean(log(df$unemployment)) -
    es * mean(log(df$gdp_capita)) +
    10 * log(df$resdevelop_gdp) -  
    5 * log(df$unemployment) +
    es * log(df$gdp_capita) + 
    rnorm(n, 0, 5)
...
- We use the full model since `sim_data()` incorporated effects of `unemployment` and `resdevelop_gdp`,
- we do not treat outliers,
- we use the level-log model specification,
- and we include country and year firm effects and also cluster our standard errors by these dimensions.
...
Roughly 40 countries, yielding 600 observations. The world bank data will have more observations than that. So, power will not be an issue in our analysis.
...
Based on the selected model one would conclude that the additional explanatory power of our independent variables in explaining life expectancy is not overwhelming once one allows for country and year fixed effects. Based on our weighted estimate, a $10 \%$ increase in GDP per capita would be associated with $`r round(calculate_weighted_estimate(weighted_ests, "est", "lb", "ub")$est, 3)` \log(1.1) \approx `r round(calculate_weighted_estimate(weighted_ests, "est", "lb", "ub")$est * log(1.1), 2)`$ years change in life expectancy.   
...
We will now exhaust all design choices and estimate our effect for each feasible combination of choices. As these combine to $11,264$ protocols, running the code below will take a while. This is why we load the data that is generated by the commented lines from the disk.

``` {r exhaust_design, results = "hide"}
ests <- exhaust_design(design,
                       "https://joachim-gassen.github.io/data/wb_condensed.csv")
save(ests, file = "case_study_ests.RData")

# load("case_study_ests.RData")
```

The coefficients will vary in magnitude by model specification. Thus, we first display coefficients relative to model type. To be able to assess the precise range of our estimates, we use histogram-based ridge lines instead of densities.
...
Not surprisingy, the magnitude of the "level-log" coefficients are much larger than the other coefficients. Let's vizualise the coefficients independently by type with different scales.

``` {r estimate_density_by_model_type_sep, fig.width = 7, fig.height = 4, fig.align = "center"}
ggplot(ests, aes(x = est)) +
  geom_histogram(fill = "lightblue", color = NA) +
  facet_grid(cols = vars(model_type), scales = "free") +
  theme_minimal() + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
```


In line with the overall shape of the Preston curve, the specifications with untransformed independent variables generate estimates closer to zero than the ones with log-transformed independent variables. For the following, we will concentrate on the level-log specification that we hypothesized.  

``` {r estimate_density_by_feffect, fig.width = 7, fig.height = 6, fig.align = "center"}
log_ests <- ests %>%
  filter(model_type == "level-log")
...
To some extent. When the outlier treatment is being applied on a smaller sample it seems to drive the coefficient up. Other then that the control variables seem to have little influence on the main coefficient. 

Finally, let's have a look at the outlier treatment.

``` {r estimate_density_by_outlier, fig.width = 7, fig.height = 6, fig.align = "center"}
rel_ests %>%
  filter(na.omit == "yes",
         idvs == "full") %>%
...
Using the `rdfanalysis` package, we are able to systematically explore the 16,232 degrees of freedom that we identified in a supposedly straight-forward panel data analysis of observational data. 

Allowing for time-invariant country-level unobservables and country-invariant unobservables affetcing the time-series, we estimate that effect of a 10 % increase of GDP per capita to be in the area of `r round(calculate_weighted_estimate(weighted_ests, "est", "lb", "ub")$lb * log(1.1), 2)` to `r round(calculate_weighted_estimate(weighted_ests, "est", "lb", "ub")$ub * log(1.1), 2)` years. Assuming that no other unobservables or reverse causality affects our findings, this coefficient can be causally interpretated.

As can be seen from rsearcher degrees of freedom exploration, our estimate is only one of many feasible estimates. In particular, the country-year fixed effet structure has a dampening effect on the analysis. Building on the level-log specification, if one would only include country-fixed effects a by 10 % increase in GDP could be associated with an increase of up to `r round(max(log_ests$ub) * log(1.1), 2)` years. On the other hand, if one uses a level-level estimation ...
